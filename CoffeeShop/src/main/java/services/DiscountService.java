...
public class DiscountService {
	...
	public BigDecimal calculatePrice(Category itemCategory, IMenuItem menuItem, int count) {
		...
		if (categoryDiscount != null && menuItemDiscount != null && categoryDiscount.sumWithOthers()
				&& menuItemDiscount.sumWithOthers()) {
			// то сначала получаем цену с учётом скидки на позицию,и умножаем на количество
			// позиций.
			BigDecimal fullPriceWithDiscount = menuItemDiscount.getDiscountPrice().multiply(bigDecimalMenuItemCount);

			// а после отнимаем скидку за категорию.
			// Скидка за категорию вычисляется с учётом уже применённой скидки за позицию
			return fullPriceWithDiscount.subtract(categoryDiscount.calculatePrice(fullPriceWithDiscount));
		}
		// Если есть только одна скидка или скидки не сумируются, то приоритетной
		// скидкой считаем скидку на позицию
		if (menuItemDiscount != null) {
			return menuItemDiscount.getDiscountPrice().multiply(bigDecimalMenuItemCount);
		}
	...
	private Optional<IDiscount> findDiscount(String promocode) {
		return discounts.stream().filter(discount -> Objects.equals(discount.getPromocode(), promocode)).findFirst();
	}
	...
	private Optional<IDiscount> findDiscount(Object object) {
		return discounts.stream().filter(discount -> discount.getPromocode() == null && discount.check(object))
				.findFirst();
	}
}