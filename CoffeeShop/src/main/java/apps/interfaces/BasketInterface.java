...
public class BasketInterface extends AbstacratInterface{
	...
	private void showBasketMenuOptions() {
		...
		if(enteredPromocode != null && !enteredPromocode.isEmpty())
		{
			...
			for(Entry<IMenuItem, Integer> entry : basketItems.entrySet())
			{
				...
				sendMessage("Item: " + menuItem.getName() + 
					". Count: " + count + 
					". Original price: " +  originalPrice + 
					". Discount price: " + priceAfterPromocode);
			}
			sendMessage("Total price: " + totalPriceAfterPromocode + 
						". Total discount: " + totalOriginalPrice.subtract(totalPriceAfterPromocode));
		...
		else
		{
			...
			if(basketItems != null && !basketItems.isEmpty())
			{
				...
				sendMessage("Total price:" + totalDiscountPrice + 
						    ". Total discount: " + totalOriginalPrice.subtract(totalDiscountPrice));
			}
			else
			{
	...
	private void enterPromoCode() {
		...
		menuItemsAfterPromocode = customerSessionInterface
			.getDiscountPrice(customerSessionInterface.getBasket(), enteredPromocode);
