...
public class OrderInterface extends AbstacratInterface {
	...
	public OrderInterface(CustomerSessionInterface customerSessionInterface) {
		...
		categories = customerSessionInterface.getCafe().getMenu().getItems().keySet().stream()
				.collect(Collectors.toMap(menuItemCategory -> categoryIndex.getAndIncrement(), menuItemCategory -> menuItemCategory));
	}
